...
class Exercisefolders extends My_Controller {
    ...
    public function index_search() {
        ...
        $data['page_title'] = 'General Routines Folders';
        $data['active_class'] = 'exercise';
        $data['isgeneral'] = $_POST["is_general"];
        $isgeneral = false;
        if($data['isgeneral'] == "1"){
			//add all existing videos of therapist
		        $getAllfolders = new Exercise_folder();
    ...
    public function addclientfolder() {
        ...
        if(!empty($listPatients))
        {
            $name = $listPatients->firstname.' '.$listPatients->lastname;
              ...
              }else{my_alert_message("danger", "Opps there is some error please try again later. Thank you");}
              redirect('admin/exercisefolders/clientroutinesfolder');
           
        } else {
            my_alert_message("danger", "You must select Client Folders to Save Routine");
            redirect('admin/exercisefolders/clientroutinesfolder');
        }
    }
...
$companyVideoFolderId = $this->db->query( $sqlVideoFolder);
$data['companyVideoFolderId']=$companyVideoFolderId;
*/

/*
			$this->db->select('select A.id as folderId, B.id as companyVideoFolderId');
			$this->db->from('exercise_folder as A');
			$this->db->join('company_video_folder as B', 'A.client_id=B.client_id', 'left');
			$this->db->where('A.id', $folderid);
			$companyVideoFolderId = $this->db->get();
